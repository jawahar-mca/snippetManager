...
export default function EntryForm({ initial, onSave }: Props) {
  ...
  // Track which blocks were manually overridden by the user
  // Existing blocks on edit are all pre-overridden (don't auto-change them)
  const [overriddenBlocks, setOverriddenBlocks] = useState<Set<string>>(
    new Set(initial?.codeBlocks.map(b => b.id) ?? [])
  )

  // When primary language changes, sync all non-overridden blocks
  useEffect(() => {
    if (isEdit) return // don't auto-sync when editing an existing entry
    setBlocks(prev =>
      prev.map(b => overriddenBlocks.has(b.id) ? b : { ...b, language })
    )
  ...
  // New block inherits current primary language and is NOT overridden yet
  function addBlock() {
    const newBlock: CodeBlock = {
      id: uuidv4(),
      language, // inherit primary language at time of clicking Add
      code: '// Write your solution here\n',
      caption: '',
    }
    setBlocks(prev => [...prev, newBlock])
    // Note: intentionally NOT adding to overriddenBlocks so it keeps syncing
  }

  function removeBlock(id: string) {
    setBlocks(prev => prev.filter(b => b.id !== id))
    setOverriddenBlocks(prev => { const s = new Set(prev); s.delete(id); return s })
  }
  ...
  function handleBlockLanguageChange(id: string, lang: Language) {
    // Mark as manually overridden — primary language changes won't touch this block
    setOverriddenBlocks(prev => new Set(prev).add(id))
    updateBlock(id, { language: lang })
  }
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-vault-dim mt-1">
              Snippets will auto-update to match unless manually changed.
            </p>
          </div>
        </div>
            ...
            <div key={block.id} className="rounded-xl border border-vault-border bg-vault-surface overflow-hidden">
              <div className="flex items-center gap-3 px-4 py-2.5 border-b border-vault-border bg-vault-card">
                ...
                <select
                  ...
                  onChange={e => handleBlockLanguageChange(block.id, e.target.value as Language)}
                ...
                {/* Show indicator if this block is overridden */}
                {overriddenBlocks.has(block.id) && (
                  <span
                    title="Language manually set — won't sync with primary language"
                    className="text-xs text-vault-dim border border-vault-border rounded px-1.5 py-0.5 cursor-help">
                    custom
                  </span>
                )}
...
//   const [errors, setErrors] = useState<Record<string, string>>({})
//   const [overriddenBlocks, setOverriddenBlocks] = useState<Set<string>>(
//     new Set(initial?.codeBlocks.map(b => b.id) ?? []) // existing blocks = already set, don't auto-change
//   )
//   function validate(): boolean {
//     const e: Record<string, string> = {}
//     if (!title.trim()) e.title = 'Title is required'
...
//   // Change addBlock to pass current language
//   function addBlock() {
//     const newBlock: CodeBlock = {
//       id: uuidv4(),
//       language, // inherit primary language at time of adding
//       code: '// Write your solution here\n',
//       caption: '',
//     }
//     setBlocks(prev => [...prev, newBlock])
//     // New blocks start as NOT overridden — primary language changes will sync to them
//   }
...
//   // When primary language changes, update all non-overridden blocks
//   useEffect(() => {
//     if (isEdit) return // don't auto-sync when editing existing entry
//     setBlocks(prev =>
//       prev.map(b =>
//         overriddenBlocks.has(b.id) ? b : { ...b, language }
//       )
//     )
//   }, [language]) // eslint-disable-line react-hooks/exhaustive-deps
//   const inputCls = 'w-full bg-vault-surface border border-vault-border rounded-lg px-3 py-2 text-sm text-vault-text placeholder:text-vault-dim/50 focus:outline-none focus:border-vault-bright/60 focus:ring-1 focus:ring-vault-bright/20 transition-all'
...
//     return {
//       id: uuidv4(),
//       language, // inherit current primary language
//       code: '// Write your solution here\n',
//       caption: '',
//     }
//   }
...
//                     setOverriddenBlocks(prev => new Set(prev).add(block.id))
