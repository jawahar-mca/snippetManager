...
// Receives id as a plain prop from the server wrapper page.tsx
export default function EntryDetail({ id }: { id: string }) {
  ...
  function renderExplanation(md: string) {
    return md.split('\n').map((line, i) => {
      if (line.startsWith('## '))
        return <h2 key={i} className="text-vault-bright font-display text-lg font-semibold mt-6 mb-2">{line.slice(3)}</h2>
      if (line.startsWith('### '))
        return <h3 key={i} className="text-vault-text font-semibold mt-4 mb-1">{line.slice(4)}</h3>
      if (line.startsWith('- '))
        return <li key={i} className="text-vault-dim text-sm ml-4 list-disc">{renderInline(line.slice(2))}</li>
            ...
            <div key={block.id} className="rounded-xl border border-vault-border bg-vault-surface overflow-hidden">
              <div className="flex items-center justify-between px-4 py-2.5 border-b border-vault-border bg-vault-card">
                <div className="flex items-center gap-2">
                  ...
                  {block.caption && (
                    <span className="text-sm text-vault-dim">{block.caption}</span>
                  )}
                </div>
